...
def _configure(apis, manifest, appname):
    ...
    try:
        existing = restclient.get(apis, '/v3/app/' + appname).json()
    ...
    if manifest:
        ...
        if existing:
            restclient.put(apis, '/v3/app/' + appname, payload=app)
        else:
            restclient.post(apis, '/v3/app/' + appname, payload=app)
...
def _delete(apis, appname):
    ...
    restclient.delete(apis, '/v3/app/' + appname)
    return None


def _list(apis):
    """List configured apps."""
    response = restclient.get(apis, '/v3/app/')
