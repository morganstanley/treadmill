...
def init():
    ...
    def vring_cmd(manifest):
        ...
        ring = app['vring']
        utils.validate(ring, [('rules', True, list), ('cells', True, list)])

        if context.GLOBAL.cell not in ring['cells']:
            _LOGGER.critical(
                'cell %s not listed in vring.', context.GLOBAL.cell
            )
            sys.exit(-1)
        ...
        if len(rules) != 1:
            _LOGGER.critical('(TODO): multiple rules are not supported.')
            sys.exit(-1)
