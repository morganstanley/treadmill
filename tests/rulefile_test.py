...
class RulefileTest(unittest.TestCase):
    ...
    def test_filename(self):
        ...
        self.assertEquals(
            rulefile.RuleMgr._filenameify(self.natrule),
            "dnat:1.1.1.1:123-2.2.2.2:234"
        )
        self.assertEquals(
    ...
    def test_get_rule(self):
        ...
        """
        self.assertEquals(
            self.rules.get_rule("dnat:1.1.1.1:123-2.2.2.2:234"),
            self.natrule,
        )
        self.assertEquals(
    ...
    def test_unlink_rule(self):
        """Test rule cleanup.
        """
        self.rules.create_rule(self.natrule, self.natuid)

        self.rules.unlink_rule(self.natrule, self.natuid)
        self.assertFalse(
            os.path.exists(
                os.path.join(self.rules_dir, self.natfile)
            )
        )
    ...
    def test_unlink_rule_not_owner(self):
        ...
        self.rules.unlink_rule(self.natrule, self.passthroughuid)
        self.assertTrue(
            os.path.exists(
                os.path.join(self.rules_dir, self.natfile)
            )
    ...
    def test_create_rule(self):
        ...
        self.assertEquals(
            ...
            os.path.join(self.apps_dir, self.natuid)
        )
    ...
    def test_create_rule_conflict(self):
        ...
        self.assertRaises(
            ...
            self.natrule, self.passthroughuid
        )
    ...
    def test_get_rules(self):
        ...
        """
        for filename in [self.natfile, self.passthroughfile, 'not_a_rule']:
            with open(os.path.join(self.rules_dir, filename), 'w+'):
                pass

        rules = self.rules.get_rules()
        self.assertIn(self.natrule, rules)
